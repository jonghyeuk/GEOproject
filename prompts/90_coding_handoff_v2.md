...
| 유형 | 정의 | 생성 경로 |
|------|------|-----------|
| **Core 문서** (= 기본문서) | 순수 정보 문서. 서비스 홍보 없음. | `00` → `01~06` → `20` → (`21`) |
| **Service-Bridge 문서** (= 연결문서) | 정보 80% + 서비스 연결 20%. Core 축적 후 생성. | `00` → `10` → `11` → `20` → (`21`) |

- 둘 다 정보 문서임. Bridge도 정보가 중심이고 서비스 언급은 보조.
- Core가 카테고리당 3개 이상 쌓인 후에만 Bridge 생성.
- "기본문서"라는 별도 3번째 유형은 없음. Core = 기본문서.
